...
def generate_database_now(path_to_source_training_package, path_to_database_training_package, file_date):
    ...
    dataset_annotations_folder = os.path.join(path_to_source_training_package, "yumi_dr_annotations")
    ...
    if not os.path.exists(dataset_annotations_folder):
        rospy.logfatal("Dataset not found==>"+str(dataset_annotations_folder)+", please launch 'roslaunch yumi_dr yumi_dr_environment.launch' + 'roslaunch yumi_dr yumi_dr.launch'")
        return False
    else:
        rospy.loginfo("Trainin annotations path found ==>"+str(dataset_annotations_folder))
    ...
    for i, xml_file in enumerate(xml_files):
        ...
        path = os.path.join(dataset_annotations_folder)
...
def main():
    ...
    if len(sys.argv) < 4:
        rospy.logfatal("usage: 'generate_dataset.py path_to_source_training_package, path_to_database_training_package'")
    else:
